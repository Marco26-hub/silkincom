...
export default function AdminCategoriePage() {
  ...
  const parentName = (id: string | null) => id ? categories.find((x) => x.id === id)?.name || '—' : '—';

  return (
    <div className="space-y-6 max-w-[1400px]">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-4xl mb-1">Categorie</h1>
          <p className="text-soft-grey text-sm">{categories.length} categorie</p>
        </div>
        {!showForm && (
      ...
      {showForm && (
        <form onSubmit={save} className="border border-pearl-grey bg-white p-6 space-y-4">
          <h2 className="text-sm font-medium mb-2">{creating ? 'Nuova categoria' : `Modifica ${editing!.name}`}</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-[10px] uppercase tracking-[0.2em] text-soft-grey mb-1.5">Categoria genitore</label>
              <select value={form.parent_id || ''} onChange={(e) => setForm({ ...form, parent_id: e.target.value })} className={cls + ' bg-white'}>
        ...
        <table className="w-full text-sm">
          <thead className="bg-warm-white border-b border-pearl-grey">
            <tr className="text-left text-[10px] uppercase tracking-[0.2em] text-soft-grey">
              ...
              <th className="px-5 py-3 font-medium">Genitore</th>
          ...
          <tbody className="divide-y divide-pearl-grey/60">
            {loading ? (
              ...
              <tr><td colSpan={6} className="px-5 py-12 text-center text-soft-grey">Nessuna categoria</td></tr>
            ) : categories.map((c) => (
              <tr key={c.id} className="hover:bg-ivory/50">
                <td className="px-5 py-3 font-medium">{c.name}</td>
                <td className="px-5 py-3 font-mono text-xs">{c.slug}</td>
                <td className="px-5 py-3 text-xs">{parentName(c.parent_id)}</td>
