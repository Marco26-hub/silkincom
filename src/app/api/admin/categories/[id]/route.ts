...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const allowed = ['name', 'slug', 'description', 'parent_id', 'display_order', 'is_active', 'seo_title', 'seo_description'];
  ...
  if ('parent_id' in updates) updates.parent_id = updates.parent_id || null;
  ...
  await logAdminAction(auth.userId, 'update_category', 'category', id, updates);
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await logAdminAction(auth.userId, 'delete_category', 'category', id, {});
  return NextResponse.json({ ok: true });
}
