...
import { discoverLeadFromWebsite, searchLeadCandidates } from '@/lib/lead-discovery';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Ricerca live non disponibile';
  ...
  for (const candidate of candidates.slice(0, input.maxResults)) {
    try {
      ...
      if (error) {
        warnings.push(`${candidate.link}: ${error.message}`);
        continue;
      }
      saved.push(data);
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Errore discovery';
      warnings.push(`${candidate.link}: ${message}`);
    }
  }
