...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: current } = await supabase
    ...
    .eq('id', id)
    .single();
  if (!current) return NextResponse.json({ error: 'Slide non trovata' }, { status: 404 });
  ...
  await logAdminAction(auth.userId, 'replace_home_slide_image', 'home_slide', id, { storage_path: storagePath });
