...
import { Search, Send, ExternalLink, Trash2, Plus, RefreshCw, Mail, Building2, Globe, ShieldCheck, MapPin } from 'lucide-react';
...
export function LeadB2BPanel({
  ...
  const [liveQuery, setLiveQuery] = useState('b&b charme relais hotel boutique resort shop spa luxury concept store');
  ...
  const [campaignNotes, setCampaignNotes] = useState('Se desidera, possiamo inviare una proposta riservata con selezione prodotto e condizioni dedicate.');
  ...
  const stats = useMemo(() => {
    ...
    const withEmail = leads.filter((lead) => Boolean(lead.contact_email)).length;
    const replied = leads.filter((lead) => Boolean(lead.last_reply_at)).length;
    const stopped = leads.filter((lead) => Boolean(lead.stop_requested_at) || lead.do_not_contact).length;
    return { total: leads.length, qualified, contacted, withEmail, replied, stopped };
  }, [leads]);
  ...
  async function runLiveSearch() {
    if (!liveQuery.trim()) {
      setMessage('Inserisci una ricerca, ad esempio "hotel lusso Lago di Como".');
      return;
    }
    ...
    try {
      ...
      setMessage(`Ricerca live completata: ${data.saved || 0} lead salvati da ${data.candidates || 0} risultati.`);
      await refresh();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'Errore ricerca live');
    } finally {
      setBusy(false);
    }
  }
  ...
  async function runDiscovery() {
    ...
    try {
      const response = await fetch('/api/admin/leads/discover', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ urls, industry: scanIndustry, notes: scanNotes }),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'Discovery fallita');
      setMessage(`Scansione completata: ${data.discovered || 0} lead aggiornati.`);
      setScanUrls('');
      await refresh();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'Errore di scansione');
    } finally {
      setBusy(false);
    }
  }
  ...
  async function addManualLead() {
    ...
    try {
      ...
      setMessage(`Lead salvato: ${data.lead?.company_name || manual.company_name}`);
      setManual({
        company_name: '',
        website_url: '',
        industry: 'hospitality',
      ...
      setMessage(error instanceof Error ? error.message : 'Errore salvataggio lead');
    } finally {
      setBusy(false);
    }
  }
  ...
  async function sendOutreach(overrideIds?: string[]) {
    ...
    try {
      ...
      setMessage(`Invio completato: ${sent} inviati${failed ? `, ${failed} saltati` : ''}.`);
      setSelectedIds([]);
      await refresh();
    } catch (error) {
        ...
        <section className="border border-pearl-grey bg-white p-5 space-y-4">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-soft-grey">Cerca aziende reali online e crea lead con sito, pagina contatti ed email pubblica quando disponibile.</p>
            </div>
            <button
              type="button"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr_140px] gap-4">
            <div className="space-y-2">
              <label className="text-[10px] uppercase tracking-[0.2em] text-soft-grey">Query</label>
            ...
            <div className="space-y-2">
              <label className="text-[10px] uppercase tracking-[0.2em] text-soft-grey">Zona</label>
              <div className="flex items-center gap-2 border border-pearl-grey bg-warm-white px-3 py-3">
            ...
            <div className="space-y-2">
              <label className="text-[10px] uppercase tracking-[0.2em] text-soft-grey">Risultati</label>
              <select
        ...
        <section className="border border-pearl-grey bg-white p-5 space-y-4">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-soft-grey">Inserisci domini hotel, pagine contatti o altri siti business pubblici.</p>
            </div>
            <button
              type="button"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2 md:col-span-2">
              <label className="text-[10px] uppercase tracking-[0.2em] text-soft-grey">URL da scansionare</label>
              <textarea
                value={scanUrls}
            ...
            <div className="space-y-2">
              <label className="text-[10px] uppercase tracking-[0.2em] text-soft-grey">Settore</label>
              <select
                value={scanIndustry}
            ...
            <div className="space-y-2">
              <label className="text-[10px] uppercase tracking-[0.2em] text-soft-grey">Note discovery</label>
              <input
                value={scanNotes}
        ...
        <section className="border border-pearl-grey bg-white p-5 space-y-4">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-soft-grey">Per importazioni rapide o contatti già verificati.</p>
            </div>
            <button
              type="button"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Field label="Azienda" value={manual.company_name} onChange={(company_name) => setManual((prev) => ({ ...prev, company_name }))} />
            <Field label="Sito web" value={manual.website_url} onChange={(website_url) => setManual((prev) => ({ ...prev, website_url }))} />
            <Field label="Città" value={manual.city} onChange={(city) => setManual((prev) => ({ ...prev, city }))} />
            <Field label="Paese" value={manual.country} onChange={(country) => setManual((prev) => ({ ...prev, country }))} />
            <Field label="Email contatto" value={manual.contact_email} onChange={(contact_email) => setManual((prev) => ({ ...prev, contact_email }))} />
            <Field label="Ruolo" value={manual.contact_role} onChange={(contact_role) => setManual((prev) => ({ ...prev, contact_role }))} />
            <Field label="Nome contatto" value={manual.contact_name} onChange={(contact_name) => setManual((prev) => ({ ...prev, contact_name }))} />
            <Field label="Telefono" value={manual.contact_phone} onChange={(contact_phone) => setManual((prev) => ({ ...prev, contact_phone }))} />
            <div className="space-y-2">
              <label className="text-[10px] uppercase tracking-[0.2em] text-soft-grey">Settore</label>
              <select
                value={manual.industry}
                onChange={(e) => setManual((prev) => ({ ...prev, industry: e.target.value }))}
                className="w-full border border-pearl-grey px-3 py-3 bg-warm-white text-sm focus:outline-none focus:border-soft-black"
              >
                {FOCUS_OPTIONS.map((option) => (
                  <option key={option.value} value={option.value}>{option.label}</option>
            ...
            <Field label="Fonte" value={manual.source_url} onChange={(source_url) => setManual((prev) => ({ ...prev, source_url }))} />
            <Field label="Pagina contatti" value={manual.public_contact_page} onChange={(public_contact_page) => setManual((prev) => ({ ...prev, public_contact_page }))} />
            <div className="space-y-2 md:col-span-2">
              <label className="text-[10px] uppercase tracking-[0.2em] text-soft-grey">Note</label>
              <textarea
                value={manual.notes}
        ...
        <section className="border border-pearl-grey bg-white overflow-hidden">
          <div className="px-5 py-4 border-b border-pearl-grey flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-soft-grey">{leads.length} record salvati</p>
            </div>
            <div className="flex gap-2">
              <button type="button" onClick={() => selectVisible(false)} className="px-3 py-2 text-sm border border-pearl-grey">Seleziona tutti</button>
              <button type="button" onClick={() => selectVisible(true)} className="px-3 py-2 text-sm border border-pearl-grey">Solo con email</button>
              <button type="button" onClick={() => setSelectedIds([])} className="px-3 py-2 text-sm border border-pearl-grey">Reset</button>
            </div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-pearl-grey/60">
                {leads.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-4 py-12 text-center text-soft-grey">
                      Nessun lead ancora. Inserisci i primi domini da scansionare.
                    </td>
                  </tr>
                ) : leads.map((lead) => (
                  <tr key={lead.id} className={selectedIds.includes(lead.id) ? 'bg-ivory/60' : ''}>
                    ...
                    <td className="px-4 py-4 align-top">
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-2 text-xs text-soft-grey">
                          ...
                          <a href={lead.website_url} target="_blank" rel="noreferrer" className="hover:text-gold-primary truncate max-w-[240px]">
                            {lead.website_url}
                          </a>
                        </div>
                        {lead.notes && <p className="text-xs text-soft-grey max-w-[420px] line-clamp-2">{lead.notes}</p>}
                    ...
                    <td className="px-4 py-4 align-top">
                      <div className="space-y-1 text-xs">
                        ...
                        <div>{lead.contact_name || '—'} {lead.contact_role ? `· ${lead.contact_role}` : ''}</div>
                        <div>{[lead.city, lead.country].filter(Boolean).join(' · ') || '—'}</div>
                        {lead.public_contact_page && (
                          <a href={lead.public_contact_page} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-gold-primary hover:underline">
                            Pagina contatti <ExternalLink className="w-3 h-3" />
                          </a>
                        )}
                      </div>
                    </td>
                    <td className="px-4 py-4 align-top">
                      <div className="space-y-2">
                        <span className={`inline-block px-2 py-0.5 text-[10px] uppercase tracking-[0.15em] rounded ${STATUS_BADGES[lead.status] || 'bg-pearl-grey text-soft-grey'}`}>
                    ...
                    <td className="px-4 py-4 align-top text-sm">
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-soft-grey">Email: {lead.email_sent_count ?? 0}</p>
                        <p className="text-xs text-soft-grey">Risposte: {lead.reply_count ?? 0}</p>
                        <p className="text-xs text-soft-grey">{lead.do_not_contact ? 'Bloccato' : 'OK'}</p>
                        {lead.stop_requested_at && (
                          <p className="text-xs text-red-700">STOP {new Date(lead.stop_requested_at).toLocaleDateString('it-IT')}</p>
                        )}
                      </div>
                    </td>
                    <td className="px-4 py-4 align-top">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => deleteLead(lead.id, lead.company_name)}
                        ...
                        <button
                          type="button"
                          onClick={() => patchLead(lead.id, { do_not_contact: !lead.do_not_contact, status: lead.do_not_contact ? 'qualified' : 'do_not_contact' })}
      ...
      <aside className="space-y-6">
        <section className="border border-pearl-grey bg-white p-5">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <label className="text-[10px] uppercase tracking-[0.2em] text-soft-grey">Focus offerta</label>
              <select
                value={focus}
            ...
            <div className="space-y-2">
              <label className="text-[10px] uppercase tracking-[0.2em] text-soft-grey">Nota campagna</label>
              <textarea
                value={campaignNotes}
            ...
            <p className="text-[11px] text-soft-grey leading-relaxed">
              Il sistema usa solo contatti business pubblici, traccia invii e risposte, e blocca automaticamente chi risponde “stop”.
            </p>
          </div>
        </section>
          ...
          {selectedLeads.length === 0 ? (
            ...
            <ul className="space-y-2">
              {selectedLeads.map((lead) => (
                <li key={lead.id} className="text-sm border-b border-pearl-grey/60 pb-2 last:border-0 last:pb-0">
                  <p className="font-medium">{lead.company_name}</p>
                  <p className="text-xs text-soft-grey">{lead.contact_email || 'email mancante'}</p>
                </li>
              ))}
            </ul>
          )}
        ...
        <section className="border border-pearl-grey bg-white p-5 space-y-3">
          ...
          {replies.length === 0 ? (
            <p className="text-sm text-soft-grey">Nessuna risposta tracciata.</p>
          ) : (
            <ul className="space-y-3">
              {replies.map((reply) => (
                <li key={reply.id} className="border-b border-pearl-grey/60 pb-3 last:border-0 last:pb-0">
                  <div className="flex items-center justify-between gap-3 mb-1">
                    <p className="text-sm font-medium truncate">
                      {reply.lead_accounts?.company_name || reply.from_email}
                    </p>
                    <span className={`text-[9px] uppercase tracking-[0.15em] px-2 py-0.5 ${reply.intent === 'stop' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
                      {reply.intent}
                    </span>
                  </div>
                  <p className="text-xs text-soft-grey truncate">{reply.subject || reply.from_email}</p>
                  {reply.message_excerpt && (
                    <p className="text-xs text-soft-black/70 line-clamp-2 mt-1">{reply.message_excerpt}</p>
                  )}
                  <p className="text-[10px] text-soft-grey mt-1">{new Date(reply.received_at).toLocaleString('it-IT')}</p>
                </li>
              ))}
            </ul>
          )}
        ...
        {message && (
          <section className="border border-pearl-grey bg-white p-5 text-sm leading-relaxed">
            {message}
          </section>
        )}
      </aside>
...
function Stat({ label, value }: { label: string; value: number }) {
  return (
    <div className="border border-pearl-grey bg-white p-4">
      <p className="text-[10px] uppercase tracking-[0.2em] text-soft-grey mb-2">{label}</p>
...
function Field({
  ...
  return (
    <div className="space-y-2">
      <label className="text-[10px] uppercase tracking-[0.2em] text-soft-grey">{label}</label>
      <input
        value={value}
